...
function calculate_error_v4()
    % calculate_error_v4.m (V1.22 - RELATIVE OFFSET CALIBRATION)
    % Objective: Resolve the CapTrak "Dummy Sphere" Trap by recovering the 
    % missing translation vector (Origin Shift) between the Cap Base Z=0 
    % and the True Anatomical Z=0.
    ...
    fprintf('\n--- PHASE 7: FINAL ERROR ANALYSIS (V1.22 ORIGIN SHIFT) ---\n');
    ...
    for i = 1:length(subjects)
        ...
        try
            ...
            % 3. RESOLVE THE ORIGIN DISCONNECT (The "Dummy Sphere" Trap Fix)
            Base_Fid  = [base.NAS(:)'; base.LPA(:)'; base.RPA(:)'];
            Truth_Fid = [truth.NAS(:)'; truth.LPA(:)'; truth.RPA(:)'];

            % Calculate the Origin Shift Vector to recover the lost Affine Translation
            % This bridges the gap between the CapTrak Origin (Cap Center) and MEG Origin (Ears)
            Origin_Shift_Vector = mean(Truth_Fid) - mean(Base_Fid);

            % Apply the Origin Shift to the predictions and the baseline
            predL_final = predL_captrak + Origin_Shift_Vector;
            predR_final = predR_captrak + Origin_Shift_Vector;
            
            baseL_final = base.LPA(:)' + Origin_Shift_Vector;
            baseR_final = base.RPA(:)' + Origin_Shift_Vector;
