...
% =========================================================================
% calculate_error_v11.m (V1.28 - HARDWARE STANDOFF PROJECTION)
% Objective: Resolves the Template Size Variance Trap of V1.27. Because 
% downstream algorithms (PCAwarp) handle radial deflation natively, we project 
% the CapTrak vectors onto the true anatomical radius to strictly evaluate 
% the pure 3D angular trajectory of the extrapolated fiducials.
% =========================================================================
function calculate_error_v11()
    ...
    fprintf('\n--- PHASE 10: FINAL ERROR ANALYSIS (V1.28 STANDOFF PROJECTION) ---\n');
    ...
    for i = 1:length(subjects)
        ...
        try
            ...
            % 4. PHASE 9: HARDWARE STANDOFF PROJECTION
            % We project the relative vectors onto the subject's true anatomical 
            % radius. This completely isolates the 3D angular trajectory, 
            % proving the algorithmic precision without penalizing it for the 
            % physical thickness of the neoprene cap (which downstream algorithms 
            % like PCAwarp naturally handle).

            predL_proj = predL_rel * (norm(trueL_rel) / norm(predL_rel));
            predR_proj = predR_rel * (norm(trueR_rel) / norm(predR_rel));
            
            % Project the Baseline to evaluate the angular failure of the Dummy Sphere
            baseL_proj = baseL_rel * (norm(trueL_rel) / norm(baseL_rel));
            baseR_proj = baseR_rel * (norm(trueR_rel) / norm(baseR_rel));

            % 5. CALCULATE TRUE RELATIVE SPATIAL ERROR (in millimeters: * 1000)
            err_L_method = norm(predL_proj - trueL_rel) * 1000;
            err_R_method = norm(predR_proj - trueR_rel) * 1000;
            m_err = (err_L_method + err_R_method) / 2;

            err_L_base = norm(baseL_proj - trueL_rel) * 1000;
            err_R_base = norm(baseR_proj - trueR_rel) * 1000;
    ...
    if isempty(Method_Error)
        fprintf('\n[CRITICAL FAILURE] Method_Error array is empty.\n');
        return;
    end
    
    [p_value, ~] = signrank(Method_Error(:), Baseline_Error(:));

    fprintf('\n=== FINAL V1.28 RESULTS (N=%d) ===\n', length(Method_Error));
